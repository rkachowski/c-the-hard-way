...
char *test_traverse()
{
    ...
    mu_assert(traverse_called == 2, "Wrong traverse count for failure.");
...
char *test_delete()
{
    ...
    mu_assert(deleted == &expect1, "Should get test1 correctly.");
    bstring result = BSTree_get(map, &test1);
    mu_assert(result == NULL, "Should delete.");

    deleted = (bstring)BSTree_delete(map, &test1);
    mu_assert(deleted == NULL, "Should get NULL on deleted.");

    deleted = (bstring)BSTree_delete(map, &test2);
    mu_assert(deleted != NULL, "Got NULL on delete.");
    mu_assert(deleted == &expect2, "Should get test2 correctly.");
    result = BSTree_get(map, &test2);
    mu_assert(result == NULL, "Should delete.");

    deleted = (bstring)BSTree_delete(map, &test3);
    mu_assert(deleted != NULL, "Got NULL on delete.");
    mu_assert(deleted == &expect3, "Should get test3 correctly.");
    result = BSTree_get(map, &test3);
    mu_assert(result == NULL, "Should delete.");
...
char *test_fuzzing()
{
    ...
    for( i = 0; i < 100; i++) {
        ...
        for(j = i+1;j < 99; j++) {
            ...
            mu_assert(value == data[j], "Failed to get the correct number.");
        }
