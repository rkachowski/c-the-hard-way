...
int Shell_run(apr_pool_t *p, Shell *cmd)
{
    ...
    check(rv == APR_SUCCESS, "Couldn't create proc attr.");

    rv = apr_procattr_io_set(attr, APR_NO_PIPE, APR_NO_PIPE, APR_NO_PIPE);
    check(rv == APR_SUCCESS, "Couldn't set io of command");

    rv = apr_procattr_dir_set(attr, cmd->dir);
    check(rv == APR_SUCCESS, "Couldn't set dir of command to %s", cmd->dir);

    rv = apr_procattr_cmdtype_set(attr, APR_PROGRAM_PATH);
    check(rv == APR_SUCCESS, "Couldn't set command type of command to %s", APR_PROGRAM_PATH);
...
Shell TAR_SH = {
    ...
    .dir = "/tmp/pkg-build",
    .required_vars = 1,
    .args = {"tar", "-xzvf", "FILE", "--strip-components", "l", NULL}
};
Shell CURL_SH = {
    .exe = "curl",
    .dir = "/tmp",
