...
int Command_depends(apr_pool_t *p, const char *path)
{
    ...
    in = fopen(path, "r");
    check(in, "Failed to open downloaded depends %s", path);

    for(line = bgets((bNgetc)fgetc, in, '\n'); line != NULL;
            line = bgets((bNgetc)fgetc, in, '\n')){
        btrimws(line);

        log_info("Processing depends data %s", bdata(line));
...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only)
{
    ...
    if( apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS){
        ...
        if(info.scheme){
            ...
            check(rc == 0, "curl failed to get %s", url);
        } else{
        ...
        check(rv == APR_SUCCESS, "Failed to create directory %s", BUILD_DIR);

        rc = Shell_exec(TAR_SH, "FILE", TAR_GZ_SRC, NULL);
        check(rc == 0, "Failed to untar file %s", TAR_GZ_SRC);
        ...
        check(rc == 0, "Failed to untar file %s", TAR_BZ2_SRC);
    } else {
...
int Command_install(apr_pool_t *p, const char *url, const char *configure_opts, const char *make_opts, const char *install_opts)
{
    ...
    if(rc == 1) {
        ...
        check(rc == 0, "Error building %s", url);
    } else if (rc == 0){
